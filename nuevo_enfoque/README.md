...
- **🎯 Alta Precisión**: Modelo CRNN optimizado con >95% de accuracy en palabras realistas
- **🚀 Alto Rendimiento**: Dataloader optimizado con caché en RAM y prefetching multi-threaded
- **🎨 Data Augmentation Avanzado**: 10+ transformaciones para mejorar generalización
- **📊 Vocabulario Extendido**: Soporte para español (incluyendo acentos) e inglés
- **🔄 Transfer Learning**: Capacidad de continuar entrenamiento desde checkpoints
- **📈 Métricas Detalladas**: Word Accuracy, CER (Character Error Rate), análisis por longitud
- **🎲 Dataset Realista**: Generador de palabras con patrones lingüísticos reales
...
```
ENTRADA (256x32x1) → CNN → RNN → CTC → SALIDA (Texto)
```
...
Conv2D(128, 3x3) + ReLU → MaxPool(2x1)
Conv2D(256, 3x3) + ReLU
```

- **Input**: (batch, 32, 256, 1) - Imágenes en escala de grises
- **Output**: (batch, 4, 64, 256) - Feature maps
- **Reducción de dimensionalidad**: 32x256 → 4x64
...
- **Output**: (batch, 64, 1024) - Secuencia de 64 time steps
...
- **Output**: (batch, 64, 256) - 256 = 128*2 (bidireccional)
...
- **Output**: (batch, 64, 75) - Probabilidades por posición temporal
...
```bash
# Genera dataset + Entrena modelo desde cero
python pipe_limpio_20k.py
```

Este script ejecuta automáticamente:
1. Genera 20,000 palabras realistas
2. Crea imágenes sintéticas con augmentation
3. Entrena modelo CRNN por 100 épocas
4. Guarda checkpoints y logs

**Tiempo estimado**: ~8-10 minutos (con GPU)
...
python generador_palabras_realistas.py -c 50000 -o palabras.txt -s 42

# Crear imágenes desde palabras
python dataset_creator_v2.py --archivo-palabras palabras.txt --output ./mi_dataset
```
...
│   └── dataloader_optimized.py       # Dataloader con caché RAM y prefetching
│
├── 🎲 Generación de Dataset
│   ├── generador_palabras_realistas.py   # Generador de palabras lingüísticas
│   └── dataset_creator_v2.py             # Creador de imágenes con augmentation
│
...
│
├── 🔄 Pipeline
│   └── pipe_limpio_20k.py            # Pipeline completo automatizado
│
├── 📂 Datos
...
1. **Preparación de Datos**
   - Carga de imágenes (256x32 grayscale)
...
**Uso**:
```bash
# Generar 20,000 palabras con seed 42
python generador_palabras_realistas.py -c 20000 -o labels.txt -s 42
...
- ✅ Variación de espaciado entre letras
...
**Configuración**:
- Dataset: 20,000 palabras realistas
- Épocas: 100
- Batch size: 96
...
50      | 0.0567     | 0.0823   | 95.8%   | 2.1%
100     | 0.0234     | 0.0512   | 96.3%   | 1.5%
```
...
| **CRNN Aether** | **96.3%** | **2.1%** | **150 img/s** |
