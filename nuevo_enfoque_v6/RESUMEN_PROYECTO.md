 ...
 **Especificaciones**
- Input: 512x32 píxeles (escala de grises)
...
##  Estado Actual (Versión 6 - Noviembre 2025)

El proyecto se encuentra en una fase avanzada de optimización (**V6**), habiendo superado las limitaciones de resolución y generalización de versiones anteriores.

###  Logros Recientes (V6)
1.  **Arquitectura High-Res**: Se modificó el stride de pooling para duplicar la resolución horizontal (256 time-steps), permitiendo leer frases largas (>45 caracteres) y URLs complejas sin pérdida de información.
2.  **Smart Padding**: Implementación de ancho mínimo (512px) en generación de imágenes para evitar la deformación de palabras cortas.
3.  **Variable Tracking**: Data augmentation con espaciado variable entre letras para mejorar la separación de caracteres.
4.  **Scheduler Dinámico**: Integración de `ReduceLROnPlateau` para ajuste automático de learning rate.
5.  **Dataset Robusto**: Distribución mejorada con más URLs, códigos alfanuméricos y casos difíciles (l/I/1, O/0).

###  Métricas de Rendimiento (Estimadas V6)
- **Word Accuracy**: >97% (Esperado)
- **CER**: <0.5%
- **Velocidad**: ~140 img/s (GPU)
- **Capacidad de Lectura**: Hasta 60-70 caracteres por línea
