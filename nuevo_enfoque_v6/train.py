...
def main(config_path=None, pretrained_weights=None):
    ...
    # Optimizer con warmup y Scheduler
    initial_lr = tcfg['learning_rate']
    warmup_epochs = tcfg.get('warmup_epochs', 5)
    
    # Configuración del Scheduler
    sched_cfg = tcfg.get('scheduler', {})
    use_scheduler = sched_cfg.get('enable', False)
    
    class ReduceLROnPlateauManual:
        def __init__(self, factor=0.5, patience=2, min_lr=1e-6, verbose=True):
            self.factor = factor
            ...
            if current_loss < self.best_loss:
                ...
                if self.wait >= self.patience:
                    ...
                    if new_lr < current_lr:
                        if self.verbose:
                            print(f"\n   Scheduler: Reduciendo LR de {current_lr:.6f} a {new_lr:.6f}")
                            print(f"     (Loss no mejoró por {self.patience} validaciones)")
                        self.wait = 0
                        return new_lr
            return current_lr
    ...
    scheduler = None
    if use_scheduler:
        print(f" Scheduler activado: ReduceLROnPlateau (patience={sched_cfg.get('patience', 2)}, factor={sched_cfg.get('factor', 0.5)})")
        scheduler = ReduceLROnPlateauManual(
    ...
    def get_current_lr(epoch, current_lr):
        # Fase 1: Warmup
        if epoch < warmup_epochs:
            return initial_lr * (epoch + 1) / warmup_epochs
        # Fase 2: Scheduler (si está activo) o constante
        return current_lr
    
    # Inicializar optimizador
    current_lr = initial_lr if warmup_epochs == 0 else (initial_lr / warmup_epochs)
    ...
    # Variable para mantener el LR actual fuera del bucle
    # (se actualizará por el scheduler)
    active_lr = initial_lr
    ...
    for epoch in range(tcfg['epochs']):
        ...
        # Update learning rate (Warmup o Mantener valor del scheduler)
        if epoch < warmup_epochs:
            active_lr = get_current_lr(epoch, initial_lr)
        
        # Asignar al optimizador
        model.optimizer.learning_rate.assign(active_lr)
        print(f" Learning Rate: {active_lr:.6f}")
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            ...
            if use_scheduler and epoch >= warmup_epochs:
                new_lr = scheduler.step(val_loss, active_lr)
                if new_lr != active_lr:
                    active_lr = new_lr
                    # Se asignará al optimizador al inicio de la siguiente época
            ...
            for i in range(5):
                ...
                print(f"     {match} Real: '{true}' | Pred: '{pred}'")
        
        # Guardar log con métricas de validación
        with open(log_file, 'a') as f:
            f.write(f"{epoch+1},{epoch_loss:.6f},{active_lr:.8f},{val_accuracy:.2f},{val_loss:.6f},{val_cer:.2f}\n")
