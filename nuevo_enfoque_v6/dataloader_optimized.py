...
class OCRDatasetOptimized:
    ...
    def _preload_images(self):
        ...
        def load_single_image(item):
            ...
            try:
                ...
                # --- SMART PADDING ---
                # 1. Resize manteniendo aspect ratio para altura self.img_h (32)
                w, h = img.size
                scale = self.img_h / h
                new_w = int(w * scale)
                img = img.resize((new_w, self.img_h), Image.BILINEAR)
                
                # 2. Crear canvas de tamaño final (self.img_w, self.img_h)
                # Rellenar con blanco (255) o negro (0)?
                # Asumimos imágenes sobre fondo claro -> Rellenar con blanco (255)
                # OJO: Si el modelo espera 0 como padding, cambiar a 0.
                # Usualmente en OCR de texto negro sobre blanco, el padding es blanco (255).
                final_img = Image.new('L', (self.img_w, self.img_h), color=255)
                
                # 3. Pegar imagen centrada o a la izquierda
                # Centrado es mejor para data augmentation implícito
                if new_w > self.img_w:
                    # Si es más grande, forzar resize (no debería pasar mucho con 40 chars)
                    img = img.resize((self.img_w, self.img_h), Image.BILINEAR)
                    final_img.paste(img, (0, 0))
                else:
                    paste_x = (self.img_w - new_w) // 2
    ...
    def _get_image(self, path):
        ...
        if self.cache_in_ram and path in self.image_cache:
            ...
            # --- SMART PADDING (Duplicado para fallback) ---
            w, h = img.size
            scale = self.img_h / h
            new_w = int(w * scale)
            img = img.resize((new_w, self.img_h), Image.BILINEAR)
