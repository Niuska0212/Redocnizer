...
try:
    ...
    generar_palabras = generar_dataset_realista
...
DEFAULT_CONFIG = {
    'cantidad_palabras': DATASET_CONFIG.get('dataset_size', 1000), # Usar dataset_size del config si existe
    'cantidad_fechas': 500,
    'longitud_min': 3,
    'longitud_max': 64,
    ...
    'augmentation': {
        ...
        'spacing_probability': 0.40,   # 40% de imágenes con espaciado modificado
        'spacing_range': (-2, 6),      # Rango de espaciado (tracking) en píxeles
        'spacing_range': (-2, 6),      # Rango de espaciado (tracking) en píxeles
        'min_width': 0,                # Ancho mínimo 0 para imágenes "tight"
    },
    'font_variations': {
...
def calcular_dimensiones_texto(
    ...
    Args:
        ...
        spacing: Espaciado adicional entre caracteres (tracking)
    ...
    # Obtener bbox del texto normal para altura
    bbox = draw.textbbox((0, 0), texto, font=font)
    text_height = bbox[3] - bbox[1]
    
    # Calcular ancho considerando espaciado
    if spacing == 0:
        text_width = bbox[2] - bbox[0]
    else:
        total_width = 0
        for char in texto:
            total_width += font.getlength(char) + spacing
        if len(texto) > 0:
            total_width -= spacing
        text_width = int(total_width)
...
def draw_text_with_spacing(
    ...
    """Dibuja texto con espaciado variable (tracking)."""
    if spacing == 0:
...
def generar_imagen_texto(
    ...
    #  VARIACIÓN 9: Espaciado variable (Tracking)
    spacing = 0
    if enable_aug and random.random() < aug_cfg.get('spacing_probability', 0):
        spacing_range = aug_cfg.get('spacing_range', (-1, 3))
        spacing = random.uniform(*spacing_range)
    
    # Calcular dimensiones con espaciado
    width, height, text_width, text_height = calcular_dimensiones_texto(
        texto, font, padding, spacing
    )
    
    #  SMART PADDING: Asegurar ancho mínimo (512px) para evitar distorsión en resize
    #  MODIFICACIÓN: Se elimina el ancho mínimo para generar imágenes "tight" (ajustadas al texto)
    #  El dataloader se encargará del padding.
    min_width = aug_cfg.get('min_width', 0) # Default cambiado a 0
    if width < min_width:
        width = min_width
    ...
    # Calcular posición centrada del texto (con variación aleatoria si hay espacio)
    if width > text_width + padding * 2:
        # Si sobra espacio, permitir cierto desplazamiento aleatorio
        max_offset = (width - text_width) - padding * 2
        offset_x = random.randint(0, int(max_offset)) if max_offset > 0 else 0
        x = padding + offset_x
    else:
    ...
    # Dibujar texto con espaciado
    draw_text_with_spacing(draw, (x, y), texto, font, text_color, spacing)
    
    #  VARIACIÓN 3: Simular bold (dibujar texto desplazado ligeramente)
    if simulate_bold:
        for offset_x, offset_y in [(1, 0), (0, 1), (1, 1)]:
            draw_text_with_spacing(draw, (x + offset_x, y + offset_y), texto, font, text_color, spacing)
...
class DatasetCreator:
    ...
    def _generar_imagenes_batch(
        ...
        for i, texto in enumerate(textos, 1):
            try:
                # Enforce max length (ahora 64)
                if len(texto) > 64:
                    # self.logger.warning(f"   ️  Texto muy largo ({len(texto)} chars): '{texto}' -> Truncando a 64")
                    texto = texto[:64]
