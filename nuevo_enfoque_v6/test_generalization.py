...
def main():
    ...
    if best_checkpoint.exists():
        ...
        print(f"   (model_best.weights.h5)")
    elif final_checkpoint.exists():
        checkpoint = final_checkpoint
        print(f" Usando: {checkpoint}")
        print(f"   (model_final.weights.h5)")
    else:
        # Fallback para buscar por épocas si no hay best/final
        checkpoints = sorted(list(checkpoint_dir.glob("model_epoch_*.weights.h5")))
        if checkpoints:
            checkpoint = checkpoints[-1]
            print(f" Usando último checkpoint encontrado: {checkpoint}")
        else:
            print(f" ERROR: No se encontró ningún checkpoint en {checkpoint_dir}")
            return
    ...
    parser = argparse.ArgumentParser(description='Test de Generalización OCR')
    parser.add_argument('--regenerate', action='store_true', help='Regenerar dataset de prueba antes de evaluar')
    ...
    should_regenerate = args.regenerate
    
    # Si no existe o si el usuario lo pide interactivamente
    if not labels_file.exists() and not should_regenerate:
        print(f"️  No se encontró dataset en {images_dir}")
        response = input("¿Deseas generarlo ahora? (s/n): ").lower()
        if response == 's':
            should_regenerate = True
    elif not should_regenerate:
        # Opción interactiva si ya existe
        print(f"ℹ️  Dataset encontrado en {images_dir}")
        response = input("¿Deseas REGENERARLO con las últimas mejoras (URLs largas, etc)? (s/n): ").lower()
        if response == 's':
            should_regenerate = True

    if should_regenerate:
        print("\n REGENERANDO DATASET DE PRUEBA (20,000 muestras)...")
        ...
        cmd = [
            ...
            "--fonts", "fonts",
            "--fonts", "fonts",
            "--longitud-max", str(cfg['dataset'].get('max_text_length', 40)), # Usar valor del config
            "--quiet" # Menos ruido
        ]
        ...
        try:
            ...
            print(f" Error al regenerar dataset: {e}")
            return
        except FileNotFoundError:
            print(" No se encontró dataset_creator_v2.py en el directorio actual")
            return

    if not labels_file.exists():
        print(f" Error: No existe el archivo de etiquetas {labels_file}")
        return
    
    
    print(f" Dataset de prueba: {test_dataset_dir}")
    
    # Cargar usando OCRDatasetOptimized (con Smart Padding)
    ...
    dataset = OCRDatasetOptimized(
        ...
        batch_size=1,
        split='train',  # Usar todo el dataset
        train_split=1.0,
        seed=42,
        augment=False,      # No augmentation para test
        cache_in_ram=True   # Acelerar lectura
    )
    ...
    for i in range(n_quick_test):
        ...
        if i < 10 or (not is_correct and i < 50):
            ...
            print(f"      {match} '{true_label}' → '{pred_label}' (ED: {ed})")
