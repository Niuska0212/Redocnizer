...
```bash
# Pipeline completo V6: genera dataset 200k + entrena
python pipe_v6.py

# Esperar ~4-6 horas (dataset grande)...
#  Modelo entrenado guardado en: dataset_entrenamiento/checkpoints/
```

### 1.3 Evaluar Modelo (Estándar de Oro)

```bash
# Test de generalización (Recomendado)
# Evalúa en 20,000 muestras NUNCA vistas, usando Smart Padding correcto.
...
python generador_palabras_realistas.py -c 200000 -o palabras.txt -s 42
...
training:
  batch_size: 64        # ️ Reducir si OOM (512px usa más RAM)
...
# Con GPU, toma ~15 min para 20K imágenes
...
======================================================================
 ...
 Steps por época (train): 219

 Modelo compilado: ~2,000,000 parámetros
...
======================================================================
 Learning Rate: 0.000333
  [████████████████████████████████████████] 100.0% | Step 219/219 | Loss:  2.3456
...
def preprocesar_imagen(imagen_path, img_w=512, img_h=32):
    """Preprocesa imagen para el modelo"""
    img = Image.open(imagen_path).convert('L')
    img = img.resize((img_w, img_h), Image.BILINEAR)
    arr = np.array(img).astype(np.float32) / 255.0
    arr = np.expand_dims(arr, axis=-1)  # (32, 512, 1)
    arr = np.expand_dims(arr, axis=0)   # (1, 32, 512, 1)
    return arr
...
# En config.yaml:
model:
  input_width: 1024  # ️ Aumentar (antes 512)
  input_height: 32
  
# En model.py, ajustar Reshape:
x = layers.Reshape((-1, 4096))(x)  # ️ 4*1024 = 4096 (antes 2048)
```
...
def entrenar_multi_gpu():
    ...
    with strategy.scope():
        ...
        base = build_crnn(input_shape=(32, 512, 1), num_classes=len(idx2char))
...
// Preparar input
float[][][][] input = new float[1][32][512][1];
// ... llenar con imagen preprocesada

// Inferencia
float[][][] output = new float[1][256][75];  // (batch, time, classes)
...
- 200K imágenes (V6), 100 épocas: ~4-6 horas
- 50K imágenes, 100 épocas: ~1-2 horas

**Sin GPU (CPU)**:
- 200K imágenes, 100 épocas: ~12-15 horas
